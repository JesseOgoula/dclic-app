  ...
  const exportToMarkdown = () => {
    ...
    if (dashboardStats) {
      ...
      if (dropoutRate > 20) {
        ...
        recommendations.push(`**[Vigilance] Décrochage** : ${dropoutRate}% de la cohorte est en décrochage. Poursuivre les relances individuelles.`);
      }

      if (blockedCount > 0) {
        recommendations.push(`**[Bloqué] ${blockedCount} apprenant${blockedCount > 1 ? 's' : ''} bloqué${blockedCount > 1 ? 's' : ''}** : Des relances et un accompagnement personnalisé sur les activités évaluées sont nécessaires.`);
      }

      if (inactiveCount > 5) {
        recommendations.push(`**[Relance] ${inactiveCount} apprenants inactifs** : Planifier des relances par mail pour réengager les apprenants.`);
      }

      if (phase1Count > 0) {
        recommendations.push(`**[Phase 1] ${phase1Count} apprenant${phase1Count > 1 ? 's ont' : ' a'} terminé la Phase 1** : Préparer l'entrée dans le Projet Professionnel.`);
      }

      if (completedCount > 0) {
        recommendations.push(`**[Clôture] ${completedCount} apprenant${completedCount > 1 ? 's ont' : ' a'} terminé la session** : Préparer les certificats et la clôture.`);
      }

      if (recommendations.length === 0) {
        recommendations.push('Aucune alerte critique. La cohorte suit un rythme régulier.');
      }
